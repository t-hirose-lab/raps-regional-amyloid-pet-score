"""
Figure 2b: Bootstrap ROI selection frequency bar chart (horizontal)
Top 20 ROIs by selection_freq_pct, 4-color scheme, square format (4.5 x 4.5 inches)
"""
...
top20 = df.nlargest(20, 'selection_freq_pct').copy()
...
top20['color'] = top20.apply(assign_color, axis=1)
...
top20['label'] = top20['roi'].apply(clean_label)

# Sort: highest freq at top -> ascending so barh puts highest at top
top20 = top20.sort_values('selection_freq_pct', ascending=True)
...
ax.barh(
    y_pos,
    top20['selection_freq_pct'].values,
...
for i, (freq, col) in enumerate(zip(top20['selection_freq_pct'].values, top20['color'].values)):
    ax.text(freq + 1.0, i, f'{freq:.1f}%', va='center', ha='left', fontsize=7, fontfamily='Arial')
...
# Panel label "B" at top-left (10pt bold) + plot title
...
out_path = '/home/takumi/pj3-adni-roi/submit_v2/figures/fig2b_bootstrap_bar.svg'
